...
const Listing = () => {
    ...
    const handleDeleteSubmit = async () => {
        try {
            setLoadingCRUD(true);
            const data = await axiosWrapper('delete', `${import.meta.env.VITE_JSONPLACEHOLDER}/posts/${selectedRowId}}`);
    ...
    const ActionsRenderer = (props) => (
        <React.Fragment>
            <Row style={{ width: '100%' }}>
                ...
                <Col lg={6} md={6} sm={6} className="d-flex justify-content-center align-items-center">
                    <button className="btn-light action-button delete-button" onClick={() => props.onDeleteClick(props.data.id)}>
    ...
    const columns = [
        ...
        { headerName: 'User ID', field: 'userId', filter: 'agNumberColumnFilter', sortable: true, unSortIcon: true },
        {
            headerName: 'Title',
            field: 'title',
        ...
        { headerName: 'Created at', field: 'createdAt', filter: 'agSetColumnFilter', sortable: true, unSortIcon: true, cellRenderer: (params) => formatDate(params.value) },
        {
            headerName: 'Actions',
    ...
    return (
        <div className="listing-page">
            ...
            <Table columns={columns} tableData={productData} onRowClicked={handleRowClick} createEntry={handleCreateClick} showCreatebtn={true} loading={loading} />
        </div>
    );
};
